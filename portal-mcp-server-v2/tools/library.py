...
async def find_in_library(
    ...
    try:
        ...
        items = await make_quix_request(ctx, "POST", "library/query", json=payload)
        ...
        result += "\nTo use a template, call `create_pipeline_from_template(template_id='...', ...)`."
        return result
    except QuixApiError as e:
        # --- Guided Error Handling ---
        return f"Error searching library for '{search_term}'. Please check your network connection and ensure the search term is correct. Try using broader search terms like 'starter', 'source', 'sink', or technology names like 'kafka', 'influxdb'. Original error: {str(e)}"
...
async def create_pipeline_from_template(
    ...
    """
    try:
        await ctx.info(f"Creating application '{application_name}' from template '{template_id}'...")
        ...
        app = await make_quix_request(ctx, "POST", "library/application", json=create_app_payload)
        ...
        if input_topic or output_topic:
            ...
            await applications.set_application_topics(ctx, app_id, input_topic, output_topic)
        ...
        # --- Guided Error Handling ---
        return f"Error creating pipeline from template '{template_id}'. Please ensure the template ID is correct and the application name is unique. You can find valid template IDs with `find_in_library()`. If topics are specified, ensure they exist with `find_topics()`. Original error: {str(e)}"
...
from tools import applications, deployments, library, topics, workspaces
...
async def find_applications(ctx: Context, search: Optional[str] = None) -> str:
    """
    <usecase>
    Finds and lists applications within the current workspace. Use this to get an overview of available applications or to find a specific application_id.
    </usecase>
    <instructions>
    You can optionally provide a 'search' term to filter the results by name or path.
    </instructions>
    """
    return await applications.find_applications(ctx, search)
...
async def get_application_details(ctx: Context, application_id: str) -> str:
    ...
    <instructions>
    You must provide a valid 'application_id', which can be found using the 'find_applications' tool.
    </instructions>
    """
    return await applications.get_application_details(ctx, application_id)
...
async def manage_application(
    ...
    return await applications.manage_application(
        ctx, action, application_id, name, path, language, delete_files
    )
...
async def get_workspace_details(ctx: Context) -> str:
    """
    <usecase>
    Retrieves details about the current workspace, including its status, branch, and broker type.
    </usecase>
    """
    return await workspaces.get_workspace_details(ctx)
...
if __name__ == "__main__":
    ...
    if not os.environ.get('QUIX_TOKEN') or not os.environ.get('QUIX_BASE_URL') or not os.environ.get('QUIX_WORKSPACE'):
        logger.error("QUIX_TOKEN, QUIX_BASE_URL, and QUIX_WORKSPACE are required. Set them via arguments or a .env file.")
        exit(1)

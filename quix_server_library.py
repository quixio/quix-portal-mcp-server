...
# Initialize FastMCP server for Quix Library
mcp = FastMCP("quix_library")
...
async def make_quix_request(
    ...
    # Replace workspace_id in any placeholders if present
    if workspace_id and "{workspaceId}" in str(json):
        # Handle nested replacement in JSON payload
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run Quix Library MCP SSE-based server')
    ...
    logger.info(f"Starting Quix Library MCP server on {args.host}:{args.port}")
    logger.info(f"Using Quix Portal at {os.environ.get('QUIX_BASE_URL')}")
    
    # Log workspace ID if available
    if os.environ.get('QUIX_WORKSPACE'):
